        ...
        if 0 < now.hour < 24  :
            ...
            if init_balance_one:
                ...
                if positions_one_one and positions_one_tow:
                    balance_one = positions_one_one[0].profit + positions_one_tow[0].profit
                    if balance_one > 10  or balance_one < 0 :
                        print(balance_one)

                    if balance_one > 17 and init_balance_one and pos == 'tow':
                        close(tickit_tow_one.order)
                        time.sleep(0.1)
                        close(tickit_tow_tow.order)

                    if balance_one > 45 and init_balance_one and pos == 'tow':
            ...
            if init_balance_tow:
                ...
                if positions_tow_one and positions_tow_tow:
                    balance_tow = positions_tow_one[0].profit + positions_tow_tow[0].profit
                    if balance_tow > 10 or balance_tow < 0:
                        print(balance_tow)

                    if balance_tow > 17 and init_balance_tow and pos == 'tow' :
                        close(tickit_one_one.order)
                        time.sleep(0.1)
                        close(tickit_one_tow.order)

                    if balance_tow > 45 and init_balance_tow and pos == 'tow' :
                        close(tickit_tow_one.order)
                        time.sleep(0.1)
                        close(tickit_tow_tow.order)
    ...
    if 0 < now.hour < 24   :
        try:
            ...
            if len(mt5.positions_get(symbol='GBPUSD_o')) == 0 and len(mt5.positions_get(symbol='EURUSD_o')) == 0 :
                tickit_one_one =  buy('GBPUSD_o', 0.85 , 0 , 0 , 'one')
                tickit_one_tow =  sell('EURUSD_o', 1 , 0 , 0 , 'one')
                tickit_tow_one =  sell('GBPUSD_o', 0.85 , 0 , 0 , 'tow')
